...
const messages = ref([]);
const chatList = ref([
  { role: "system", content: `### 指令 你被设定为一个能够准确理解和生成高质量中文内容的大规模语言模型。你的任务是在中文语境下回答问题，并且需要确保回答的准确性和严谨性。请严格按照以下要求执行： 1. **语言要求**：所有回答必须使用标准的现代汉语，且语法正确、表达清晰。 2. **内容要求**： - 提供详细且准确的信息。 - 避免使用模糊不清的表述。 - 对于技术性问题，确保术语的准确性。 - 如果有数据支持，请引用可靠的来源。 3. **格式要求**： - 使用段落分隔不同的论点或信息块。 - 使用列表来组织相关项目或步骤。 - 在回答的结尾总结主要观点。 #### 回答要求 - 请确保信息的准确性，并引用可靠的历史资料。 - 请在回答中使用正式的语言风格，并确保语法准确无误。 ### 结束语 请按照上述要求，使用中文详细回答以上问题。如果有任何不确定的地方，请明确指出并提供可能的解决方案。`},
]);
const input = ref("你好");
const chatID = ref("")
